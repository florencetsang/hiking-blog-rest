...
public class PostgresDatabaseDAO implements BaseDatabaseDAO {
    ...
    public List<Trip> getTrips(String uid, int tripId) {
        final Connection conn = connectionResolver.getConnection();
        if (conn == null) {
        ...
        try (PreparedStatement preparedStatement = conn.prepareStatement(sqlStringBuilder.toString())) {
            int stmtParamIdx = 1;
    ...
    public int addTrip(String name, String description, String route, List<Integer> tagIds, String uid) {
        final Connection connection = connectionResolver.getConnection();
        if (connection == null) {
            return -1;
        }
